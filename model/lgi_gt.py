...
class LGI_GT(nn.Module): 
    ...
    def init_node_edge_encoders(self): 
        ...
        elif self.dataset_name == 'CIRCDIS':
            if self.use_gconv and self.use_tlayer:
                default_encoder_out_dim = self.gconv_dim
            elif self.use_gconv and not self.use_tlayer:
                default_encoder_out_dim = self.gconv_dim
            elif not self.use_gconv and self.use_tlayer:
                default_encoder_out_dim = self.tlayer_dim
            else:
                default_encoder_out_dim = self.gconv_dim

            if self.embedding_dim is not None:
                encoder_out_dim = self.embedding_dim
            else:
                encoder_out_dim = default_encoder_out_dim

            self.node_encoder = nn.Linear(self.in_dim, encoder_out_dim)

            self.embedding_proj = None
            if self.embedding_dim is not None:
                if self.use_gconv and encoder_out_dim != self.gconv_dim:
                    self.embedding_proj = nn.Linear(encoder_out_dim, self.gconv_dim)
                elif not self.use_gconv and self.use_tlayer and encoder_out_dim != self.tlayer_dim:
                    self.embedding_proj = nn.Linear(encoder_out_dim, self.tlayer_dim)
    ...
    def reset_parameters(self): 
        ...
        if self.dataset_name == 'ogbg-molpcba': 
            ...
            if hasattr(self, 'embedding_proj') and self.embedding_proj is not None:
                self.embedding_proj.reset_parameters()
    ...
    def forward(self, data: Union[Data, Batch]) -> Tensor: 
        ...
        elif self.dataset_name in ['PATTERN', 'CLUSTER', 'ZINC']: 
            rw = data.pestat_RWSE 
            x = x.squeeze(-1) 
            x = self.node_encoder(x) 
